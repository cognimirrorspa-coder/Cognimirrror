...
export default function BluetoothScanner({ isOpen, onClose, onConnected }) {
  ...
  const connectDevice = async () => {
    ...
    try {
      ...
      console.error(e);
      setStatus('idle');
      if (e.name !== 'NotFoundError' || e.message.includes('not available') || e.message.includes('not present')) {
        setErrorMsg(e.message.includes('not available') || e.message.includes('not present')
          ? 'El Bluetooth está desactivado. Por favor, actívalo en tu PC.'
